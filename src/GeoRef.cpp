...
void GEO::setup_maximum_flow(DGraph &dg)
{
	...
	for (e = estart; e != eend; e++)
	{
		...
		DEdge de = dg[*e];
		const double area = de.full_length; //this will become a function later
		const double flow = area * area * area * (std::max<double>(hs - ht, 0)) / de.length;
// 		const double bflow = area * area * area * (std::max(ht - hs, 0)) / de.length;
		dg[*e].capacity = flow;
